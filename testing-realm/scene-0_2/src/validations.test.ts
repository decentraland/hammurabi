export * from '@dcl/sdk'
import { EngineInfo, Entity, GltfContainer, GltfContainerLoadingState, LoadingState, MeshRenderer, Transform, engine } from '@dcl/sdk/ecs'
...
test('ensure that cubes are in the scene on the first frame', function* () {
  ...
  assertEquals(loadingStates.length, towers.length, 'The number of GltfContainerLoadingState must be equal to the number of GltfContainer')
  for (const [entity, loadingState] of loadingStates) {
    assertEquals(loadingState.currentState, LoadingState.FINISHED, `The loading state of the entity ${entity} must be FINISHED`)
  }
