...
export function getEmoteRepresentation(emote: EmoteDefinition, bodyShape: string = BodyShape.MALE) {
  const representation = emote.emoteDataADR74.representations.find((representation) =>
    representation.bodyShapes.includes(bodyShape as any)
  )
  if (!representation) {
...
export function getWearableRepresentation(wearable: Wearable, bodyShape: string = BodyShape.MALE) {
...
export function getWearableRepresentationOrDefault(definition: Wearable, shape = BodyShape.MALE) {
...
export function hasWearableRepresentation(definition: Wearable, shape: string = BodyShape.MALE) {
  try {
